...
3. В методе `onResume()`, как и в случае с потоком, мы запускаем нашу корутину, в которой используется *suspend* функция `delay`, заменяющая `Thread.sleep`. Для запуска корутины используется метод `launchWhenResumed()` из сторонней библиотеки `lifecycle`.
